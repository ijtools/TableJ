...
public class IndexedStringColumn extends AbstractColumn implements CategoricalColumn, Iterable<String>
{
    ...
    {
        super(name);
        // TODO: if initialized with 0, can refer to non-existing label...
    ...
    public IndexedStringColumn(String name, int[] indices, String[] levelNames)
    {
        super(name);
        
        this.levelIndices = indices;
        
        this.levelLabels = new ArrayList<String>(levelNames.length);
        for (String levelName : levelNames)
        {
            this.levelLabels.add(levelName);
        }
    ...
    {
        int nc = levelLabels.size();
        this.levelColors = new ArrayList<Color>(nc);
        if (nc > 0)
        {
            for (int i = 0; i < nc; i++)
            {
                int[] rgb = glasbeyDarkLut[i % nc];
                this.levelColors.add(new Color(rgb[0], rgb[1], rgb[2]));
            }
        }
    }
    ...
    {
        int n = levelCount();
        if (levelColors.size() != n)
        {
            initializeColors();
        }
        return levelColors.toArray(new Color[0]);
    }
    ...
    private static final int[][] glasbeyDarkLut = {
